...
def GenerateConditionLists(FACTOR_headingpool, FACTOR_widthpool, TrialsPerCondition):
	"""Based on two factor lists and TrialsPerCondition, create a factorial design and return trialarray and condition lists"""

	NCndts = len(FACTOR_headingpool) * len(FACTOR_widthpool)	
#	ConditionList = range(NCndts) 

	#automatically generate factor lists so you can adjust levels using the FACTOR variables
	ConditionList_heading = np.repeat(FACTOR_headingpool, len(FACTOR_widthpool)	)
	ConditionList_width = np.tile(FACTOR_widthpool, len(FACTOR_headingpool)	)

	print(ConditionList_heading)
	print(ConditionList_width)
	...
	return (TRIALSEQ_signed, ConditionList_heading, ConditionList_width)
...
class myExperiment(viz.EventClass):
	...
	def __init__(self, eyetracking, practice, tiling, exp_id, ppid = 1):
		...
		####### DATA SAVING ######
		datacolumns = ['ppid', 'heading', 'cameraoffset', 'width','trialn','timestamp','trialtype_signed','World_x','World_z','WorldYaw','SWV','SWA','YawRate_seconds','TurnAngle_frames','Distance_frames','dt', 'StraightVisible', 'setpoint']
	...
	def RecordData(self):
		...
		if self.SAVEDATA:
			#datacolumns = ['ppid', 'heading', 'cameraoffset', 'width','trialn','timestamp','trialtype_signed','World_x','World_z','WorldYaw','SWA','BendVisible']
			output = [self.PP_id, self.Trial_heading, self.Trial_Camera_Offset, self.Trial_width, self.Trial_N, self.Current_Time, self.Trial_trialtype_signed, 
